...
select * from test where name='张三' and sex=sex123;  // 可以命中
...
- 普通索引的 不等于(!= <>) 不会走索引
- - 理论上不等于是不会命中索引的
- - 通过以上生成的假数据经测试都会命中type为range的索引（生成的数据都是按照顺序生成的，所以会命中）
- - 如果是主键或者索引是整数类型，会命中索引
```
...
>本文章来源于网络,略有修改，如有侵权行为，请[点击这里](https://github.com/mattmengCooper/MattMeng_hexo/issues)联系我删除
