...
>gz命令
    ...
    gzip -c test.txt > /root/test.gz # 指定压缩的路径并保留源文件
    压缩：gzip FileName
    .tar.gz 和 .tgz
...
- - 1.sytemctl status nginxReboot.service 后状态为activating (start)而不是activating (running)态
```
...
- 7.加到fstab文件中让系统引导时自动启动（防止重启后swap分区变成0）
```
vi /etc/fstab
// 末尾加入
