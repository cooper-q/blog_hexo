...
## 修改Change时间
- linux无法直接修改Change时间，但是可以通过修改本地时间重新创建文件来修改
```
# 执行下条命令修改本地时间
date -s "Fri 17 Sep 2021 06:18:54 AM UTC"
...
```
# 查询是否开放
$(find /usr/bin /usr/sbin | grep -w semanage) port -l | grep http_port | grep  443
...
```
getenforce
# permissive 宽容模式、enforcing 强制模式
```
...
# 105./usr/bin /usr/local/bin区别
- usr 指 Unix System Resource
- /usr/bin下面的都是系统预装的可执行程序，会随着系统升级而改变
- /usr/local/bin目录是给用户放置自己的可执行程序的地方，推荐放在这里，不会被系统升级而覆盖同名文件。
- 优先级一般是 /usr/local/bin > /usr/bin

# 106./bin /sbin /usr/sbin /usr/bin
## 1./sbin
- 基本命令，如shutdown reboot

## 2./bin
- 普通的基本命令 如ls chmod

## 3.区别
- 从用户权限的角度 /sbin目录下的命令通常只有管理员才可以运行，/bin下的命令 管理员和一般用户都可以使用
- 从可运行的角度/sbin /bin能够挂载其他文件系统前可以使用

## 4./usr/bin,/usr/sbin与/sbin /bin目录的区别在于
...
- /bin是系统指令。bin为binary的简写主要放置一些系统的必备执行档例如:cat、cp、chmod df、dmesg、gzip、kill、ls、mkdir、more、mount、rm、su、tar等
...
# 107.DoH查询域名ip
>[文档](https://developers.cloudflare.com/1.1.1.1/dns-over-https/json-format)
- ipv4
```
curl -s -H 'accept:application/dns-json' 'https://cloudflare-dns.com/dns-query?name='www.baidu.com'&type=A' | jq -r .Answer[0].data
```

- ipv6
```
curl -s -H 'accept:application/dns-json' 'https://cloudflare-dns.com/dns-query?name='www.baidu.com'&type=AAAA' | jq -r .Answer[0].data
```

# 108.iperf测试两台服务器间的带宽
...
- 3.结果
```
[root@server5 ~]# iperf -s
------------------------------------------------------------
Server listening on TCP port 5001
TCP window size: 85.3 KByte (default)
------------------------------------------------------------
[  4] local 192.168.1.2 port 5001 connected with 192.168.1.3 port 54990
...
root     pts/2    192.168.1.200    14:21    0.00s  0.01s  0.00s w
```
...
- 查找符合条件的文件内容的文件[输出文件路径]
```
# type 类型 一般正规文件(f)、设备文件(b,c)、目录(d)、连接文件(l)、socket（s）及FIFO(p)等
find . -type f|xargs grep -ls 1234
```

# 115.批量替换文件内容为空格(多文件)
- -i原地替换 因为替换会直接替换源文件比较危险，为了规避需要为替换后的文件加一个后缀
```
# 替换后的文件加一个.bak后缀
sed -i ".bak" "s/源码之家//g" $(find ./* -type f|xargs grep -ls 源码之家)

# 如果"" 则不备份
sed -i "" "s/源码之家//g" $(find ./* -type f|xargs grep -ls 源码之家)

sed -i "" "s/更多模板//g" $(grep -rl 更多模板 ./*)

sed -i "" "s/网页模板//g" $(find ./* -type f|xargs grep -ls 网页模板)

# 批量替换文件内容
find . -type f -exec sed -i 's/https:\/\/paas\.gotgo\.cc:30443/https:\/\/pass\.extrotec\.com:30443/g' {} +

```
# 116.记录命令操作日志
```
export HISTORY_FILE=/var/lg/`date '+%y-%m-%d'`.log
export PROMPT_COMMAND='{ date "+%y-%m-%d %T ##### $(who am i |awk "{print \$1\" \"\$2\" \"\$5}") #### $(pwd) #### $(history 1 | { read x cmd; echo "$cmd"; })"; } >> $HISTORY_FILE'
```
# 117.恢复rm后的数据
```
https://cloud.tencent.com/developer/article/1684479
http://www.bewindoweb.com/207.html
...
# 120.修改时间
```
date -s  "Sat Sep 18 17:41:59 CST 2021"
```
...
# 122.#!/usr/bin/env bash和#!/usr/bin/bash、#!/bin/bash比较
## #!/usr/bin/env bash 提供了更多的灵活性
- 用户不需要去寻找程序在系统中的位置
- 不同的系统命令或者程序存放的位置可能是不同的，只要程序在你的$PATH里面，就可以查找到
...
- 使用
```
runlike -p containerID/containerName
```
# 126.apt
```
...
apt -o Dpkg::Options::="--force-confold" -y install xx
```
# 127.-bash: !": event not found错误
```
!有特殊含义转椅或者单引号
```
# 129.if条件判断
...
# 输出的日期和时间格式应该是类似于以下格式,其中EDT是当前时区的缩写。

Tue Mar 25 01:23:45 EDT 2023
